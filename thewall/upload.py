...
def handle_upload_data(uploaded_file, workers):
    fhand = uploaded_file.read().decode("utf-8").strip()
    fhand = fhand.split("\n")
    ...
    if workers == -1:
        ...
        work_single_thread(fhand)
    else:
        # Multi-threaded version
        work_multi_thread(fhand, workers)


def work_multi_thread(fhand, workers):
    work_queue = deque()
    ready_queue = []
